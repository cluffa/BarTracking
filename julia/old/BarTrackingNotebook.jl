...
# ╔═╡ a998cfbf-8e4a-467a-9007-8b252be025c6
using Flux, CUDA, BSON, NNlib, Images, Colors, VideoIO, StatsPlots, Base.Threads

# ╔═╡ 7d3b11f5-40e4-49b5-ba1c-e22b5a8f2931
md"""
## Data
"""

# ╔═╡ cf8c7b91-9876-4852-ad54-925012abd1c7
md"""
## Models
"""

# ╔═╡ 80ae77f3-a4cf-4e94-b1bf-53819fdfbb06
begin
	Core.eval(Main, :(import Flux))
	Core.eval(Main, :(import NNlib))
	BSON.@load "regmodel.bson" m
	boxmod = m
	BSON.@load "model.bson" m
	m = m # for cell reload
end;

# ╔═╡ 2d177e70-f7eb-4c18-8ab0-b23045e21255
md"""
## Functions
"""

# ╔═╡ a40ef196-a15e-4df2-8672-7b21fa5a09f8
function show_pred(img, pred::Array{Float32, 3})
	out = cat(
		Float32.(Gray.(imresize(img, (116,116)))),
		pred[:,:,1],
		pred[:,:,2],
		dims = 3)
	colorview(RGB, permutedims(out, (3, 1, 2)))
end

# ╔═╡ 7c0a0c10-b208-4109-acd9-29f6550b7906
#make_pred(img::Array{Float32, 4}) = ((img |> gpu) |> (m |> gpu))[:, :, :] |> cpu;
make_pred(img::Array{Float32, 4}) = (img |> m)[:, :, :];

# ╔═╡ e271da92-f3f4-4950-81a1-a4889c70e2f3
make_pred(img::Array{RGB{Float32}, 2}) = permutedims(channelview(img), (2, 3, 1)) |> make_pred;

# ╔═╡ 19ce8d72-7ab6-44bf-913d-7d41ef8b2c4d
make_pred(img::Array{Float32, 3}) = reshape(img, size(img)..., 1) |> make_pred;

# ╔═╡ e0d1e441-ad48-4dba-8827-9ad98d038655
function predict(img)
	show_pred(img, make_pred(img))
end

# ╔═╡ 18e134da-5457-402d-bde8-a8dc953a71db
array_float32_video(video) = convert(Array{Array{RGB{Float32}}}, video);

# ╔═╡ d8047d5b-6404-4ca0-805d-79a45ea809f9
begin
	video = VideoIO.load("test.mp4") |> array_float32_video
	video = map(x -> imresize(x, (300, 300)), video)
	#video = cat(video..., dims = 3)
	frames = convert.(Array{Float32, 3}, channelview.(video))
	frames = cat(frames..., dims = 4)
	frames = permutedims(frames, (2, 3, 1, 4))
end;

# ╔═╡ 9dc2fb40-f708-4837-8e34-9d270188aa64
begin
	input = video[1:10:end]
	preds = []
	@threads for (idx, frame) in collect(enumerate(input))
		push!(preds, predict(frame))
	end
end

# ╔═╡ a315fa33-bb7a-4805-8344-c8a608b476ce
preds
...
git-tree-sha1 = "d10a083fbec15c5ded457366cc76240a5b3db279"
